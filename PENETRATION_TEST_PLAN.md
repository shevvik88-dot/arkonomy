...
| Requirement | Verified | Result | Severity | Fix status |
|---|---|---|---|---|
| 2.1 Full fetch-URL audit, all 23 functions | **Tested 2026-08-21** | 17 of 23 functions call `fetch()` directly (plus 2 shared helpers, `_shared/appCheck.ts` and `_shared/marketSnapshot.ts`). Traced every one back to its host/path source: `market-data` (2.3), Alpaca (`BASE_URL` hardcoded const), all Plaid functions (`plaidBase` from `Deno.env.get('PLAID_ENV')`, server-only — `plaid-get-accounts`'s `itemPlaidBase` from `plaid_items.plaid_environment`, a DB column with `DEFAULT 'production'` and NOT in the client-writable GRANT list), `ai-chat`/`stock-ai-analysis` (hardcoded `api.anthropic.com`), `weekly-report`/`large-transaction-alert`/`generate-monthly-report` (hardcoded `api.resend.com`), `alpaca-oauth-callback` (hardcoded `api.alpaca.markets`), `appCheck.ts` (hardcoded Google JWKS URL), `marketSnapshot.ts` (fixed 4-symbol array) — all clean, no client input reaches a fetch host/path. Also closed 2.2's open side-note: `plaid-link-token`'s client `redirect_uri` rides in the POST **body** to a fixed `plaidBase` host, never the fetch target itself — confirmed not SSRF (Plaid-side open-redirect risk at most, out of scope here). **Real finding: `push-notify`.** `sendPushNotification()` calls `webpush.sendNotification(subscription, ...)`, which does its own raw `fetch()` to `subscription.endpoint` with zero validation. `subscription` comes from `profiles.push_subscription` — confirmed client-writable (`docs/security-log.md`'s column-GRANT list) via direct PostgREST update, bypassing the legitimate browser `PushManager` flow (`src/hooks/usePushNotifications.js`) entirely. `push-notify`'s Mode 1 (`POST {user_id}`) lets an authenticated user trigger a send to their own stored subscription synchronously, on demand — no cron wait needed. Net effect: an authenticated user can make the edge function issue a server-side HTTP request to a host/port/path of their choosing — a live SSRF primitive, not theoretical (code-confirmed chain, not yet live-fired against a real internal target — user opted to fix directly instead of live-testing against a real address). | **MEDIUM-HIGH** — authenticated SSRF primitive (internal network probing / blind outbound proxy from the edge function's own network), gated behind a real Supabase session, not anonymous. | **Fixed, deployed** (`supabase/functions/push-notify/index.ts`, v57→v58): `sendPushNotification()` now rejects any `subscription.endpoint` whose scheme isn't `https:` or whose hostname isn't on a fixed allow-list of real push-service hosts (`fcm.googleapis.com`, `updates.push.services.mozilla.com`, `web.push.apple.com`, `*.notify.windows.com`) via a new `isAllowedPushEndpoint()` check, throwing a distinct `BlockedPushEndpointError` before any `webpush`/`fetch()` call. All 4 call sites (direct-notify, recurring-charge batch, savings-reminder batch, scheduled-payment batch) catch it distinctly via `reportIfBlockedEndpoint()`, log a Sentry **warning** (not error — could be a legitimate new push vendor, not necessarily an attack) with the blocked hostname + user id, and report `blocked_endpoint`/`invalid_endpoint` instead of falling through to the generic failure/expired-subscription paths. Deploy verified independently via `mcp__supabase__get_edge_function` (not just the CLI's own "Deployed" message) — returned file content matches the fix, version 58. **Not yet live-fired** (no real request sent to a non-allow-listed endpoint against the deployed function) — code-level fix confirmed, live confirmation of the block would need a follow-up pass. |
