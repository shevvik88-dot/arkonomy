...
| CORS | All 23 edge functions' `Access-Control-Allow-Origin` handling | No wildcard anywhere. Shared `_shared/cors.ts` resolver (`resolveCorsHeaders`) introduced 2026-09-06; allow-list = prod origin + Vercel-preview regex, fallback to prod (never echoes an arbitrary origin). Migrated so far (14 of 23): the original 5 (`auth-login`, `check-bank-connection`, `market-data`, `plaid-get-accounts`, `get-insights`) + `alpaca-portfolio`, `auth-signup`, `daily-lesson-v2`, `financial-diagnosis`, `generate-monthly-report` (inline copies) + batch 1 on the shared module (`plaid-link-token`, `plaid-exchange-token`, `plaid-refresh-balance`, `plaid-sync-transactions` — deployed + live-verified 3-origin 2026-09-06). Remaining 9 (`ai-chat`, `alpaca-invest`, `alpaca-oauth-start`, `delete-account`, `stock-ai-analysis`, `stripe-checkout`, `large-transaction-alert`, `plaid-batch-sync`, `push-notify`, `stripe-webhook`, `weekly-report`, `_shared/rateLimit.ts`) still on the static single origin — a functionality gap (breaks preview URLs), not a security hole. See BACKLOG.md CORS item for batch plan + 5 side-findings. | I4 |
