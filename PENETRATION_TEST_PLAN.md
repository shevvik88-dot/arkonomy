...
| CORS | All 23 edge functions' `Access-Control-Allow-Origin` handling | No wildcard anywhere. Shared `_shared/cors.ts` resolver (`resolveCorsHeaders`) introduced 2026-09-06; allow-list = prod origin + Vercel-preview regex, fallback to prod (never echoes an arbitrary origin). Resolver also sets `Vary: Origin` + `Cache-Control: no-store` unconditionally, and takes a `prodOnly` flag (drops the preview regex — used by `delete-account`). Migrated so far (26 of 26 functions + `_shared/rateLimit.ts`): the original 5 (`auth-login`, `check-bank-connection`, `market-data`, `plaid-get-accounts`, `get-insights`) + `alpaca-portfolio`, `auth-signup`, `daily-lesson-v2`, `financial-diagnosis`, `generate-monthly-report` (inline copies) + batch 1 (`plaid-link-token`, `plaid-exchange-token`, `plaid-refresh-balance`, `plaid-sync-transactions`) + batch 2 (`alpaca-invest`, `alpaca-oauth-start`, `stripe-checkout`, `delete-account` — last `prodOnly`) + batch 3 (`ai-chat`, `stock-ai-analysis`; `_shared/rateLimit.ts` `enforceRateLimit` now takes `{ corsHeaders }`, wired for ai-chat + stock-ai-analysis) + batch 4 (`large-transaction-alert`, `plaid-batch-sync`, `weekly-report`, `stripe-webhook` — all `prodOnly`; `push-notify` — default resolver, it IS browser-callable from App.jsx/Savings.jsx with a user JWT). Batch 5 (2026-09-07, final) collapsed all 10 inline `resolveCorsHeaders` copies onto `_shared/cors.ts` and wired the last 5 `enforceRateLimit` call sites — **every edge function + `_shared/rateLimit.ts` now goes through the one resolver**, no inline copy, no static single-origin fallback anywhere (`enforceRateLimit`'s `corsHeaders` is now a required param). `auth-login`/`auth-signup` keep localhost via `extraOrigins` (2026-08-28 incident); `cors.ts` exports `PROD_ORIGIN` as the single source (also the `auth-signup` GoTrue `redirect_to`). All 5 batches deployed + live-verified 3-origin; batch 3 & 5 additionally live-verified a real 429 echoes the caller origin. **CORS is closed** — no wildcard, no arbitrary-origin reflection, `Vary: Origin` + `Cache-Control: no-store` on every response. See BACKLOG.md CORS item for batch history + side-findings (incl. one HIGH: `App.jsx:961` `deleteAccount()` ignores `res.ok`). | I4 |
