...
| Requirement | Verified | Result | Severity | Fix status |
|---|---|---|---|---|
| 3.1 Amount edge values (all entry points) | Not yet tested | — | — | — |
| 3.2 Server-side limit enforcement | Not yet tested | — | — | — |
| 3.3 Out-of-order API call chains | **Tested 2026-08-17** | Failed — disposable test account: simulated a pending `investments` row (same shape `alpaca-invest` writes before calling Alpaca), then ran real `delete-account`. Confirmed `investments`/`profiles`/`auth.users` all correctly deleted. Then simulated the two UPDATEs that would run *after* delete-account if a real financial operation completed post-deletion (the actual race window between two independent edge function invocations): `alpaca-invest`'s order-confirm UPDATE and `stripe-webhook`'s `checkout.session.completed` UPDATE. Both matched 0 rows with `error: null` — completely silent no-op, no exception, no Sentry capture (both handlers only log `if (error)`, and there is none). | **HIGH** — real money (Alpaca trade or Stripe subscription charge) can move with zero application-side record and zero alerting if account deletion races either in-flight operation. | Not fixed |
| 3.4 Missing CHECK constraints, real impact | **Tested 2026-08-17** | Failed — negative, zero, and string `"NaN"` all inserted into `transactions.amount` via real RLS-scoped client path (anon key + real session, not service role), confirmed with test rows then cleaned up. `"Infinity"` correctly rejected (`22003 numeric field overflow` — Postgres `NUMERIC` doesn't support Infinity, only `NaN`). `NaN` is the standout: Postgres `NUMERIC` natively supports it as a value, and it's arithmetically contagious (`NaN + X = NaN`) — any `SUM()`/aggregate over that user's `transactions.amount` (Health Score, Cash Flow Forecast, get-insights, weekly-report) silently corrupts for that one account. No cross-user impact found (all known batch jobs compute per-profile, don't mix users' rows). | **NaN: MEDIUM-HIGH** (self-contained per-account computation corruption). **Negative/zero: LOW-MEDIUM** (data integrity, same-user impact only, no cross-user path found). | Not fixed — awaiting decision on CHECK constraint addition (see finding write-up in chat) |
...
| 6.2 Payment-flow-abandonment chain | **Tested 2026-08-17** | Same test as 3.3 confirms this chain concretely — see 3.3's result. | **HIGH** (same finding as 3.3) | Not fixed |
