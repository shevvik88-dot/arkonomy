...
| CORS | All 23 edge functions' `Access-Control-Allow-Origin` handling | No wildcard anywhere. Shared `_shared/cors.ts` resolver (`resolveCorsHeaders`) introduced 2026-09-06; allow-list = prod origin + Vercel-preview regex, fallback to prod (never echoes an arbitrary origin). Resolver also sets `Vary: Origin` + `Cache-Control: no-store` unconditionally, and takes a `prodOnly` flag (drops the preview regex — used by `delete-account`). Migrated so far (18 of 23): the original 5 (`auth-login`, `check-bank-connection`, `market-data`, `plaid-get-accounts`, `get-insights`) + `alpaca-portfolio`, `auth-signup`, `daily-lesson-v2`, `financial-diagnosis`, `generate-monthly-report` (inline copies) + batch 1 on the shared module (`plaid-link-token`, `plaid-exchange-token`, `plaid-refresh-balance`, `plaid-sync-transactions`) + batch 2 (`alpaca-invest`, `alpaca-oauth-start`, `stripe-checkout`, `delete-account` — the last `prodOnly`). Both batches deployed + live-verified 3-origin 2026-09-06. Remaining: `ai-chat`, `stock-ai-analysis` (batch 3), `large-transaction-alert`, `plaid-batch-sync`, `push-notify`, `stripe-webhook`, `weekly-report` (batch 4, non-browser — CORS is dead code there), `_shared/rateLimit.ts`, and the ~10 inline copies (batch 5). No security hole in the remainder — a functionality gap (breaks preview URLs). See BACKLOG.md CORS item for batch plan + 9 side-findings (incl. one HIGH: `App.jsx:961` `deleteAccount()` ignores `res.ok`). | I4 |
