...
- [ ] **"Safe to move" считается ЧЕТЫРЬМЯ разными формулами на четырёх поверхностях — кросс-surface дрейф, тот же класс, что уже унифицированные Transfer-исключение и Net Worth (найдено 2026-09-05 при разборе двух багов ai-chat, подтверждено обоими ревью-субагентами).** `App.jsx:~1783` / `Dashboard.jsx:1438` — `max(0, min(income − spend − BUFFER, balance − BUFFER))` (Dashboard берёт `income`/`spend` пропами из App, т.е. те же `effectiveIncome`/`totalSpent`, — эти два согласованы между собой). `Insights.jsx:1042-1043` — `max(0, min(monthlySavings − BUFFER, plaidBalance − BUFFER))` (база — `monthlySavings`, не `income − spend`; плюс локальный `const BUFFER = 1000` вместо импорта из `shared/financialConstants.js` — нарушение правила «не дублировать shared-константу»). `Savings.jsx:526` — `max(0, availableBalance − safetyBuffer)` (только баланс, доход/траты вообще не участвуют). `get-insights/index.ts:354-355` — `effectiveMonthlyIncome − currentMonthSpend − BUFFER`, где `effectiveMonthlyIncome` (index.ts:239-261) имеет ТОТ ЖЕ баг 2a, что чинили сегодня в App.jsx (fallback на последний месяц с доходом + ещё третий tier — историческое среднее), и всё ещё `: Infinity` при отсутствии баланса. Это `availableSafe` течёт в `computeRecommendedAmount` и в `breakdown.available`, которое юзер видит в savings-карточке Insights. **Побочный эффект сегодняшнего фикса**: до него чат и Insights-карточка были неправильны ОДИНАКОВО; теперь чат (только App.jsx-ветка) чинён — учитывает estimated-доход и null-баланс — а Insights-карточка нет, поэтому в сценарии «день 5, зарплата ещё не пришла» они видимо расходятся (чат: "$X unavailable / c поправкой"; Insights: старое завышенное число). Полноценный фикс — вынести одну `computeSafeToMove({ effectiveIncome, spend, currentBalance, buffer })` в `src/shared/financialConstants.js` (рядом с `calculateNetWorth`) + её зеркало в `supabase/functions/_shared/financialConstants.ts`, подключить во всех четырёх местах, разобравшись какая из трёх баз (`income − spend` vs `monthlySavings` vs только баланс) семантически правильная, и протянуть туда же фиксы 2a/2b. Отдельный заход, как и унификации Transfer/Net Worth ранее — не точечная правка.
