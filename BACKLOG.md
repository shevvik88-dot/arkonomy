...
### 16. Аудит экранов — Dashboard/Home ЗАКРЫТ 2026-07-18, Transactions ЗАКРЫТ 2026-07-19, Insights ЗАКРЫТ 2026-07-20, Recurring ЗАКРЫТ 2026-07-20, Savings ЗАКРЫТ 2026-07-20, Markets/StockDetail ЗАКРЫТ 2026-07-20, дальше по очереди: Profile/Settings
...
Тот же шаблон (три категории, ничего не чинить без подтверждения, коммит после каждой группы перед следующей — см. CLAUDE.md Coding rules) — на следующий экран: **Profile/Settings**.
...
### 18. Расширить Sentry-мониторинг на остальные edge functions (высокий приоритет — ЗАКРЫТ, средний — в очереди)
...
- [ ] **`notification_preferences.include_market_update` — тумблер убран из Profile.jsx UI, поле в БД есть, бэкенд не читает.** Найдено при аудите Profile/Settings (2026-07-20) — `weekly-report` не имеет вообще никакого обращения к рыночным данным (ни Finnhub, ни Alpaca). Реализация — новая интеграция: вызов `market-data` edge function изнутри `weekly-report` (или дублирование его логики), новая HTML-секция письма. `include_upcoming_bills` (тот же класс "тумблер ничего не делает") реализован в той же сессии — переиспользовал уже существующий `_shared/recurringDetector.ts`, `include_market_update` такого готового источника не имеет.
...
- [ ] **`GoalCard`'s `monthlyRate` (Savings.jsx) не привязан к конкретной цели — задваивание прогноза при 2+ целях.** Найдено при аудите экрана Savings (2026-07-20). Проекция даты достижения цели суммирует ВСЕ `type==="income" && category_name==="Transfer"` транзакции текущего месяца — в транзакциях нет `goal_id`, поэтому эта сумма независимо считается "своей" каждой картой цели. При 2+ активных целях второй (и последующие) прогноз будет завышен на ту же сумму, что уже учла первая. Даже для целей, привязанных к реальному банковскому счёту (`linkedAccount`), прогноз строится на этой общей эвристике, а не на реальной динамике именно этого счёта. Правильный фикс требует структурного изменения (per-goal атрибуция трансферов — либо `goal_id` в схеме транзакций, либо отдельная таблица переводов) — не быстрая правка, отдельная задача.
- [ ] **`#7C6BFF` ("Pro"/upgrade-акцент) захардкожен в 6 файлах проекта** (`App.jsx`, `BottomNav.jsx`, `Markets.jsx`, `Profile.jsx`, `Savings.jsx`, `UpgradeModal.jsx`), нигде не заведён именованной константой в `colors.js`'s `C`. Найдено при аудите Savings (2026-07-20) — консолидация в `C.proAccent` выходит за рамки одного экрана, нужен отдельный кросс-файловый заход.
- [ ] **`#F5C842`/`#F5A623` (Alpaca-предупреждения) захардкожены и в `Markets.jsx`, и в `App.jsx`** — тот же класс, что `#7C6BFF` выше, найдено при аудите Markets/StockDetail (2026-07-20). `#F5C842` — заголовок "Authorize Alpaca" (Markets.jsx) + Alpaca-toast "add funds" состояние (App.jsx:1655/1657/1670) — оба контекста явно Alpaca-related, консолидация вероятно осмысленна. `#F5A623` — не-US тикер warning (Markets.jsx:1110) + иконка в upgrade/trial-блоке (App.jsx:1596-1597) — тематическая связь НЕ подтверждена, нужно сначала посмотреть, один ли это смысловой цвет или совпадение, прежде чем заводить общую константу. Третий цвет из того же кластера, `#C8B86A` (муted-текст того же Alpaca-блока), оказался локальным только для Markets.jsx — уже исправлен на именованную `C.alpacaWarningMuted`, не в этом пункте.
- [ ] **Баланс/транзакции для non-OAuth банков (Bank of America подтверждён) обновляются по расписанию Plaid, не мгновенно — ограничение платформы, не баг приложения.** Расследовано 2026-07-17 (жалоба на лаг баланса "до суток"): временный debug-хук (`/item/get` + `/institutions/get_by_id` через Plaid, тот же lifecycle что `__sentryTest`, убран после проверки) подтвердил — webhook зарегистрирован и реально срабатывает (`status.transactions.last_webhook` заполнен), но `update_type: "background"` на самом Item означает, что Plaid сам решает, когда переопрашивать банк, независимо от того, когда мы дёргаем `/transactions/sync`. `/transactions/sync` отдаёт то, что Plaid уже закэшировал у себя — НЕ форсирует свежий опрос банка. Для non-OAuth/credentials-based институтов (как Bank of America) это может быть раз в сутки на стороне Plaid. Путь исправления — `/transactions/refresh` (форсирует опрос сейчас), но НЕ как замена автосинка: асинхронный (не возвращает данные сразу, только запускает переопрос + webhook по готовности) и жёстко лимитирован Plaid per-Item в сутки. Решение — задача #19 выше ("Refresh balance now" кнопка по запросу юзера, с client-side cooldown), не текущий приоритет.
...
- [ ] **CSP `connect-src` не разрешает `fonts.gstatic.com` — блокирует service worker.** Найдено 2026-07-17 (побочно, при диагностике Sentry frontend-теста): скриншот DevTools Console показал CSP-нарушение на `fonts.gstatic.com`, инициатор `sw.js:78`. `font-src` этот домен разрешает (обычная загрузка шрифтов через `@font-face` работает), но `connect-src` — нет, а service worker обращается к шрифтам через `fetch`/Cache API (типичный паттерн офлайн-кэширования шрифтов в PWA), что подпадает под `connect-src`, не `font-src`. Не связано с Sentry-задачей — отдельный, самостоятельный баг в `vercel.json`. Не исправлено.
