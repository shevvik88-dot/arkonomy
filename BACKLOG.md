...
### 11. Email-кампания Free → Pro (компенсация конверсии после anti-steering фикса на iOS) — ПРИОРИТЕТ ПОВЫШЕН 2026-08-03

**Повышено с "когда дойдут руки" до реального приоритета** по итогам аудита воронки Free→Pro (2026-08-03): подтверждено кодом (`App.jsx:1589-1714`, все точки входа в апгрейд обёрнуты `!IS_IOS_NATIVE`), что на iOS после anti-steering фикса **нет вообще ни одного пути оформить Pro внутри приложения** — ни на онбординге, ни при истечении триала (модалка "Your free trial ended" на iOS показывает только "AI insights, investing, and more are included with Pro." + кнопку Close, без единого упоминания цены или апгрейда). Это не гипотетическая проблема "когда-нибудь понадобится" — это единственный канал конверсии для всей iOS-аудитории уже сейчас, каждый день без него — упущенная конверсия trial→paid на iOS.
...
### 21. Перевести 12 уроков Today's Lesson на RU/ES/PT

```
Найдено 2026-08-12: Today's Lesson контент (src/utils/lessons.js) —
English-only с самого запуска (2026-08-08), уже отмечено в CLAUDE.md
Known issues. UI-обёртка вокруг урока (заголовок "Урок дня", streak-
подпись, "Совет") уже локализована — не переведён именно контент.
Временный честный фикс живёт в проде с 2026-08-12: если
i18n.language !== 'en', в шите урока показывается явная пометка
"этот урок доступен только на английском" вместо молчаливого показа
английского текста под русским/испанским/португальским интерфейсом.

ЗАДАЧА: перевести весь контент 12 уроков на RU/ES/PT — заголовок +
тело + совет × 12 уроков × 3 языка = 36 текстовых блоков. Это
финансовое обучение, не декоративный текст — перевод должен быть
смысловым (проверка финансовой терминологии на корректность и
естественность на каждом языке), а не механическим машинным
переводом один-в-один. Решить при реализации: хранить как per-
language объекты внутри lessons.js (парируя структуру LESSONS) или
вынести в отдельные json-файлы по образцу src/locales/*/. Покажи план
и структуру хранения перед реализацией — после неё временную пометку
"English only" убрать для RU/ES/PT.
```
...
- **iOS-сборка конвертирует Free → Pro только на US App Store storefront — ОБНОВЛЕНО 2026-08-03.** Изначальный anti-steering фикс (Guideline 3.1.3, 10 июля, "Вариант 3") убирал из iOS вообще любое упоминание цены/слова "upgrade"/веба. После апрельского 2025 постановления суда по делу Epic v. Apple (обязало Apple разрешить внешние ссылки на оплату без комиссии для US storefront — см. коммит `cd281c4`) это ослаблено: реальная кнопка "Upgrade to Pro — $9.99/mo" вернулась во все 6 прежних точек входа (App.jsx-баннер, Trial Expired Modal, Profile/Markets/Savings/Insights), но **только когда `StorefrontPlugin.swift` (StoreKit 2 `Storefront.current`) подтверждает US storefront** — device locale/region сознательно отклонены как ненадёжный прокси, не совпадающий с billing-storefront Apple ID. Кнопка ведёт на `app.arkonomy.com/upgrade` через `@capacitor/browser` (системный Safari, не in-app WebView), с промежуточным "You're leaving the app..." confirmation sheet. Не-US iOS storefronts — поведение не изменилось, по-прежнему чистый канал удержания, не приобретения (компенсация — задача 11 ниже, актуальность снижена для US-аудитории, но не для остальных).
  - **Xcode-интеграция ещё не выполнена и не верифицирована** — `StorefrontPlugin.swift` написан и лежит в `ios/App/App/`, но ещё не добавлен в Xcode target, `npx cap sync ios` не запускался, компиляция StoreKit 2 кода не подтверждена. Разработка велась на Windows-машине без Xcode/macOS — этот шаг сознательно отложен до отдельного захода на Mac (см. коммит `cd281c4` сообщение). До этого момента фича не готова к сборке/отправке в App Review.
  - **Юридическая предпосылка (сам факт постановления и его текущая формулировка в Guideline 3.1.1(a)/3.1.3(a)) не была независимо перепроверена мной** — принято со слов пользователя и общих знаний об Epic v. Apple; долгосрочный Вариант 1 (полноценный Apple IAP) из чеклиста остаётся резервным путём, если это постановление/его трактовка Apple окажется неприменимо или изменится.

---

## 🧷 ТЕХДОЛГ — зафиксировано, не срочно

- [ ] **Унификация "Safe to move" — один `computeSafeToMove()` вместо четырёх формул + двух разных buffer'ов (найдено 2026-09-05 при разборе двух багов ai-chat; PR #87 закрыл BUG 1 и точечно 2a/2b в ai-chat-ветке, остальное — сюда). Тот же класс, что уже унифицированные Transfer-исключение и Net Worth. Отдельный заход, не точечная правка.**

  **(c) Четыре разошедшиеся формулы на четырёх поверхностях:**
  - `App.jsx:~1783` / `Dashboard.jsx:1438` — `max(0, min(income − spend − BUFFER, balance − BUFFER))` (Dashboard берёт `income`/`spend` пропами из App — те же `effectiveIncome`/`totalSpent`, между собой согласованы).
  - `Insights.jsx:1042-1043` — `max(0, min(monthlySavings − BUFFER, plaidBalance − BUFFER))` (база — `monthlySavings`, не `income − spend`; плюс локальный `const BUFFER = 1000` — третья несинхронизированная копия константы, нарушение правила «не дублировать shared-константу»).
  - `Savings.jsx:526` — `max(0, availableBalance − safetyBuffer)` (только баланс, доход/траты не участвуют вообще).
  - `get-insights/index.ts:354-355` — `effectiveMonthlyIncome − currentMonthSpend − BUFFER`, где `effectiveMonthlyIncome` (index.ts:239-261) имеет ТОТ ЖЕ баг 2a (fallback на последний месяц с доходом + третий tier — историческое среднее) и всё ещё `: Infinity` при отсутствии баланса. `availableSafe` течёт в `computeRecommendedAmount` и в `breakdown.available` (видит юзер в savings-карточке Insights). **Побочный эффект PR #87**: до него чат и Insights-карточка были неправильны ОДИНАКОВО; теперь ai-chat учитывает estimated-доход и null-баланс, а Insights-карточка нет → в сценарии «день 5, зарплата не пришла» видимо расходятся.

  **(d) Формула вычитает spend-*за-прошедшие-дни*, а не ожидаемый расход/невыплаченные commitments за остаток месяца.** Проверено на реальном аккаунте 2026-09-05: `income-so-far $2487.98 − spend-so-far $174.26 − $1000 = $1313.72` → "$1,314 safe to move", хотя за остаток сентября ещё предстоит ~$4,300 расхода (реальный август: $4,526; одни только commitments $3,164). Арифметика верна, экономически число завышено в начале месяца. Единственный backstop — `min(…, balance − BUFFER)` — здесь не сработал (баланс $2,379 достаточно высок). Это и есть исходная жалоба «$1,314 значительно выше реально доступных денег» — механизм ОТДЕЛЬНЫЙ от 2a/2b. Фикс: вычитать невыплаченные за текущий месяц commitments / прораченный ожидаемый расход, не только фактический spend-to-date.

  **(e) BUFFER — плоские $1,000 для всех + конфликт с Savings.jsx.** `BUFFER = 1000` (хардкод, `src/shared/financialConstants.js:8` + зеркало `supabase/functions/_shared/financialConstants.ts:7` + третья локальная копия `Insights.jsx:1042`). Происхождение: попал в git через `9a587f3` «commit deployed work from prior sessions», обоснования «почему именно $1,000» ни в одном коммите нет — круглый дефолт. Никакой персонализации: юзер с тратами $500/мес и юзер с $8,000/мес получают одинаковый пол. Единственное исключение — `Savings.jsx:525` использует ДРУГОЙ buffer: `Math.min(500, availableBalance * 0.5)` (масштабируется от баланса, кап $500) — тот же класс cross-surface рассогласования. Значение видно юзеру: `get-insights` шлёт `bufferAmount: BUFFER`, Insights-карточка рендерит `"keeps ~$1,000 buffer"` (`insights.keeps_buffer`) — изменение затронет user-facing текст. Buffer НЕ убирать (идея «не предлагать тратить последнюю подушку» верна). При сборке `computeSafeToMove()` сделать buffer производным от реального среднемесячного расхода, напр. `max($1,000, 0.5 × avg monthly expenses)`, и одновременно убрать расхождение с `Savings.jsx` — одна buffer-логика везде.

  **Целевая форма:** одна `computeSafeToMove({ effectiveIncome, spend, currentBalance, avgMonthlyExpenses, unpaidCommitmentsThisMonth })` в `src/shared/financialConstants.js` (рядом с `calculateNetWorth`) + зеркало в `supabase/functions/_shared/financialConstants.ts`; buffer внутри неё — производный; подключить во всех четырёх местах, определив какая база (`income − spend` vs `monthlySavings` vs только баланс) семантически правильная; протянуть туда же фиксы 2a/2b из PR #87.
...
- [x] **ЗАКРЫТО 2026-09-07 (партии 1–5, PR #90–94). Найдено 2026-08-06: 23 файла (22 edge functions + `_shared/rateLimit.ts`) используют одинаковый hardcoded single-origin CORS-паттерн** (`'Access-Control-Allow-Origin': Deno.env.get('APP_URL') ?? 'https://app.arkonomy.com'`) — работает для прода, но ломается на любом non-prod origin (preview deployments, будущие staging-окружения). **UPD 2026-09-06 — заведён общий хелпер `_shared/cors.ts` (`resolveCorsHeaders(req, opts?)`); партия 1 из плана мигрировала на него `plaid-link-token` (v62), `plaid-exchange-token` (v61), `plaid-refresh-balance` (v22), `plaid-sync-transactions` (v85). Все 4 задеплоены, `get_edge_function`-verified, live-verified на 3 origin (preview → эхо, `evil.example.com` → prod-fallback, prod → без изменений). preview-regex распространён и на Plaid-минтинг токенов (паритет с `plaid-get-accounts`; JWT остаётся основной защитой — подтверждено пользователем). Инлайн-копии в ~10 уже-мигрированных функциях НЕ схлопнуты — партия 5. `_shared/rateLimit.ts` вынесен из партии 1 (опция без единого call-site = мёртвый код): CORS-резолв там подключается в партии 3 (`ai-chat`/`stock-ai-analysis`) и партии 5 (остальные 5). Партия 2 (2026-09-06, оба ревьюера — "ship with minor fixes"): мигрированы `alpaca-invest`, `alpaca-oauth-start`, `stripe-checkout`, `delete-account`. В `cors.ts` добавлены: `prodOnly` (сбрасывает preview-regex — используется `delete-account`, чтобы preview-сборка не могла удалить реальный аккаунт против прод-данных) и безусловный `Cache-Control: no-store` (было только в inline-копии `alpaca-portfolio`). 4 функции партии 1 передеплоены с обновлённым `cors.ts` (устранён дрейф версий). `stripe-checkout` остаётся на preview (паритет, JWT — основная защита). Партия 3 (2026-09-06, оба ревьюера — "ship with minor fixes"): мигрированы `ai-chat` (+ `extraAllowHeaders: x-firebase-appcheck`), `stock-ai-analysis`; `_shared/rateLimit.ts` `enforceRateLimit` принимает `{ corsHeaders }` (готовые заголовки от вызывающего — тот же контракт, что у `enforceIpRateLimit`/`requirePaidPlan`; НЕ `{ req }` — иначе 429 терял бы `prodOnly`/`extraOrigins`), проведён на этих 2 call-site. **429/503 у остальных 5 call-site (`daily-lesson-v2`, `financial-diagnosis`, `market-data`, `get-insights`, `alpaca-portfolio`) всё ещё уходят с prod-origin, хотя все прочие ответы этих функций эхо-ят preview — известный пробел, каждая правка = один `{ corsHeaders: <локальный const> }`, не регрессия (пре-существующее). Провести в партии 5 (или отдельным заходом) вместе со схлопыванием инлайн-копий.** Партия 4 (2026-09-07, оба ревьюера — "ship with minor fixes"): мигрированы `large-transaction-alert`, `plaid-batch-sync`, `weekly-report`, `stripe-webhook` — все с `{ prodOnly: true }` (не вызываются из `src/` — cron/webhook/service-role only, preview-origin там не легитимен); `push-notify` мигрирован на **дефолтный** резолвер (prod + preview) — вопреки первоначальной пометке «не-browser» он **реально вызывается из браузера** с user-JWT (`App.jsx:1018` large-tx/budget-exceeded пуши, `Savings.jsx:94` savings-reminders → CORS-preflight живой). `weekly-report`: `x-firebase-appcheck` убран из allow-header (никто не шлёт, App Check — no-op). Партия 5 (2026-09-07, финальная, оба ревьюера — "ship with minor fixes"): схлопнуты все 10 инлайн-копий `resolveCorsHeaders` на `_shared/cors.ts` (−224/+33). `auth-login`/`auth-signup` получили `{ extraOrigins: [/^http:\/\/localhost:\d+$/] }` — localhost-carve-out из инцидента 2026-08-28 сохранён end-to-end (code-reviewer прогнал хендлеры в Node-шиме: OPTIONS + форсированные 429/503 + adversarial origins — все корректны). Проведены оставшиеся 5 `enforceRateLimit` call-site → **все 7 несут `corsHeaders`**; `enforceRateLimit`'s `corsHeaders` сделан обязательным, статичный fallback удалён (тот же footgun, ради устранения которого всё и затевалось). `cors.ts`: `PROD_ORIGIN` экспортирован (единый источник для `auth-signup` redirect_to), caller-regex в `extraOrigins` нормализуется от `g`/`y`-флага. `generate-monthly-report` — дефолтный резолвер (не `prodOnly`): его manual-path триггерят с preview для верификации (2026-09-02). **Ничего не осталось — все 26 функций + `_shared/rateLimit.ts` на общем резолвере.** Найдено при попытке живой визуальной проверки Dashboard-редизайна на Vercel preview: `bankConnected` некорректно уходил в `false` (CORS-блок на `check-bank-connection`), Markets-карточка падала с "Failed to fetch" (CORS-блок на `market-data`) — оба выглядели как баги редизайна, но были найдены как CORS-баги на соседних, не тронутых редизайном функциях. **5 из 23 уже починены allow-list/regex-паттерном** (echo origin только если совпадает с прод-доменом ИЛИ `/^https:\/\/arkonomy-[a-z0-9-]+-shevvik88-dots-projects\.vercel\.app$/` — дефис в классе символов обязателен, иначе branch-alias превью типа `arkonomy-git-fix-…-shevvik88-dots-projects.vercel.app` не матчятся; иначе fallback на прод — никогда не эхо произвольного origin): `auth-login`, `check-bank-connection`, `market-data`, `plaid-get-accounts`, `get-insights` (последняя — вероятная причина, почему Coach-блок редизайна не рендерился на preview: `insight`/`allInsights` зависят от этой функции; CORS-preflight подтверждён живо, но фактический рендер Coach-блока на preview после фикса ещё не перепроверен визуально в браузере). Каждый живо проверен (реальный OPTIONS-preflight против задеплоенной функции) на 3 origin: prod (без изменений), preview (теперь разрешён), untrusted (по-прежнему падает на прод-фоллбек, не эхо). **Остаются непочиненными (17 функций + 1 shared-файл, тем же способом, отдельным заходом, каждая с той же живой проверкой, не пакетно)**: `ai-chat`, `alpaca-invest`, `alpaca-oauth-start`, `alpaca-portfolio`, `delete-account`, `generate-monthly-report`, `large-transaction-alert`, `plaid-batch-sync`, `plaid-exchange-token`, `plaid-link-token`, `plaid-refresh-balance`, `plaid-sync-transactions`, `push-notify`, `stock-ai-analysis`, `stripe-checkout`, `stripe-webhook`, `weekly-report`, `_shared/rateLimit.ts`. (18 непочиненных итого — не 13, как предполагалось в моменте; счёт сверен точно с grep по всем 23 файлам на каждом шаге, не оценкой на глаз.) Про `_shared/rateLimit.ts` отдельно: это shared-хелпер, импортируется несколькими функциями (`market-data`, `ai-chat`, `get-insights` как минимум) — даже после фикса `market-data`'s собственного CORS-константа, ответ конкретно от `enforceRateLimit()` (429 при рейт-лимите) всё ещё несёт старый хардкод, пока сам `rateLimit.ts` не починен отдельно — узкий, но реальный остаточный пробел, не полностью закрытый фиксом `market-data`.

- [ ] **App Check мёртв end-to-end (найдено 2026-09-06 при CORS-партии 1, code-reviewer).** Заголовок `x-firebase-appcheck` фигурирует в CORS allow-list ряда функций (`plaid-sync-transactions`, `get-insights`, `ai-chat`, `weekly-report`), клиентский геттер токена `src/lib/appCheck.js` существует, серверный верификатор `_shared/appCheck.ts` существует — но геттер нигде не вызывается (браузер заголовок не шлёт), а `verifyAppCheck`/аналог нигде не импортируется (сервер заголовок не проверяет). То есть защита выглядит подключённой на всех трёх слоях (клиент-либа + JWKS-верификатор + CORS allow-header) и не enforce'ит ничего ни на одном эндпоинте. Не чинить в CORS-партиях — allow-header оставлен для паритета. Решить отдельно: либо подключить реальный вызов `verifyAppCheck` на чувствительных функциях, либо выпилить клиентскую либу + верификатор + `x-firebase-appcheck` из allow-list как dead surface. Второй такой случай «задокументированная/выглядящая-живой защита фактически отключена» в проекте (первый — verify_jwt-рассогласование cron-функций, закрыто 2026-07-25) → в CLAUDE.md добавлено правило про периодическую верификацию задокументированной защиты.
- [ ] **Нет rate-limit на `plaid-link-token` / `plaid-exchange-token` / `plaid-sync-transactions` (найдено 2026-09-06, security-auditor).** Ни одной записи в `RATE_LIMITS` (`_shared/rateLimit.ts`). Авторизованный юзер может циклить создание link-token или `plaid-sync-transactions`, где каждый вызов фанится в пагинированный `/transactions/sync` + `/accounts/get` на item против Plaid-квоты. `plaid-refresh-balance` защищён своим `check_and_set_balance_refresh` cooldown (fail-closed) — единственный из четырёх. Пре-существующее, не введено CORS-фиксом.
- [ ] **`institution_name` / `institution_id` в `plaid-exchange-token` пишутся в `plaid_items` без валидации (найдено 2026-09-06, security-auditor).** `public_token` имеет presence-check, а эти два идут body → БД через `?? null` без проверки типа/длины. Не-строка (object/array) или безразмерная строка принимаются. `institution_name` потом отдаётся в UI через `check-bank-connection` (React экранирует — не XSS, но stored-garbage/bloat вектор). Фикс: `typeof x === 'string' && x.length <= 128` или reject.
- [ ] **Не-constant-time сравнение service-role ключа — консистентный паттерн в `plaid-sync-transactions`, `plaid-batch-sync:154`, `large-transaction-alert:90`, `weekly-report:53`, `push-notify:134` (найдено 2026-09-06/07, security-auditor).** `token !== Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')` / `===`. Пре-существующее; remote timing side-channel на `!==` через HTTP непрактичен, но для `plaid-sync-transactions` это единственный гейт перед cross-tenant delete. `timingSafeEqual` по UTF-8 байтам — правка на две строки, чистый follow-up на все 5.
- [ ] **`.replace('Bearer ', '')` без `.trim()` — `large-transaction-alert:83`, `weekly-report:46` (найдено 2026-09-07, security-auditor).** `plaid-batch-sync:152` имеет `.trim()`, эти два — нет. Fails safe (хвостовой пробел → `isCron` false → `auth.getUser()` → 401), но при консистентном проходе по этим функциям стоит выровнять.
- [ ] **`push-notify:281` логирует ошибку web-push с полным `subscription.endpoint` в логи функции (найдено 2026-09-07, security-auditor, needs review).** URL содержит FCM/APNs registration token. Не банковский токен и сам по себе не переиспользуем (для отправки нужны ещё `auth`/`p256dh` + VAPID private key), но это device push-credential в plaintext-логах. Пре-существующее, не тронуто CORS-партией.
- [ ] **Нет `cron.schedule`-миграции для `weekly-report` и `push-notify` (найдено 2026-09-07, code-reviewer).** `grep 'cron.schedule' supabase/migrations/*.sql` → 3 джобы (`arkonomy-monthly-report`, `arkonomy-daily-sync`, large-transaction-alert), ни одной для этих двух — расписание создано вручную в дашборде, не отслеживается в репо. Комментарии в коде про «Sunday 08:00 UTC» / «09:00» были непроверяемыми утверждениями (в партии 4 конкретное время из `weekly-report` убрано). Завести миграции с фактическим расписанием.
- [ ] **`plaid-webhook` — единственная оставшаяся функция без записи в `config.toml` (найдено 2026-09-07).** Задеплоена `verify_jwt: false` (Plaid-вебхук, ES256-подпись проверяется в теле), записи нет. `plaid-get-accounts` (та же ситуация) добавлена в партии 5, три cron-функции — в партии 4. `plaid-webhook` не трогалась ни одной CORS-партией, поэтому вынесена сюда: добавить `[functions.plaid-webhook]\nverify_jwt = false` — тогда `config.toml` полностью соответствует задеплоенному состоянию всех 26 функций, и обычный `supabase functions deploy` ничего не перевернёт.
- [x] **ЗАКРЫТО 2026-09-07 (партия 5). 7 уже-мигрированных на allow-list функций без `Vary: Origin` (найдено 2026-09-06, оба ревьюера).** `check-bank-connection`, `plaid-get-accounts`, `market-data`, `auth-login`, `auth-signup`, `get-insights`, `generate-monthly-report` эхо-или динамический origin без `Vary: Origin`. После схлопывания на `_shared/cors.ts` (который всегда ставит `Vary: Origin` + `Cache-Control: no-store`) — закрыто; code-reviewer подтвердил runtime-прогоном все 7 эмитят `Vary: Origin`.

- [ ] **HIGH — `src/App.jsx:961` `deleteAccount()` не проверяет `res.ok` и безусловно разлогинивает + сбрасывает состояние (найдено 2026-09-06, оба ревьюера при CORS-партии 2).** `await fetch('.../delete-account', …)` без `res.ok`-чека и без чтения тела; `catch` логирует только при `import.meta.env.DEV`, после чего функция безусловно чистит кэши, вызывает `signOut` и ресетит весь стейт. `delete-account` легитимно возвращает **409** (`index.ts:127`, инвестиция ещё pending), **500** (`index.ts:229`, `admin.deleteUser` упал; `index.ts:123`, состояние не удалось проверить) — и в каждом из этих случаев юзер разлогинен и видит "аккаунт удалён", хотя Stripe-подписка, Plaid-items и все строки на месте. Прямое нарушение правила CLAUDE.md («никогда не игнорировать `error` из ответа клиента БД с молчаливым `finally`, закрывающим UI независимо от результата»). Побочно: делает любой CORS-сбой на этом эндпоинте невидимым (fetch reject → тот же silent path). **Заслуживает отдельного фикса скоро, не откладывать надолго**: проверять `res.ok`, парсить тело, показывать конкретную ошибку и НЕ разлогинивать/не сбрасывать состояние при неуспехе.
- [ ] **`alpaca-invest:247`/`:260` — сырое тело ошибки Alpaca логируется И возвращается клиенту (найдено 2026-09-06, security-auditor).** `console.error('Alpaca order error:', JSON.stringify(order))` + `details: order` в ответ браузеру. Тот же файл на ~50 строк выше (`:199-208`) сознательно этого НЕ делает (возвращает `brokerage_account_error` вместо сырого тела, со ссылкой на фиксы `financial-diagnosis`/`daily-lesson-v2` и находку на `alpaca-portfolio` 2026-08-24). Alpaca 422-тела могут содержать состояние счёта (buying power, существующее количество). Фикс: логировать только `order?.code`/`order?.message`, возвращать непрозрачный `{ error: 'order_failed' }`.
- [ ] **`alpaca-invest:214` — guard buying-power fail-**open** на кривом ответе Alpaca (найдено 2026-09-06, security-auditor).** `const buyingPower = parseFloat(account.buying_power); if (buyingPower < Number(amount))` — если поле отсутствует/не число, `parseFloat` → `NaN`, `NaN < x` → `false`, ордер проходит без проверки. `requirePaidPlan.ts:16-17` прямо требует, чтобы money-moving гейты фейлились closed. Фикс: `if (!Number.isFinite(buyingPower) || buyingPower < numAmount) { await releasePending(); return 400; }`.
- [ ] **`alpaca-invest` — нет rate-limit на единственном money-**moving** эндпоинте (найдено 2026-09-06, security-auditor).** Read-only `alpaca-portfolio` имеет 60/hr (`_shared/rateLimit.ts:26`); `alpaca-invest` — ничего. `window_bucket` unique-индекс подавляет только идентичный `(symbol, amount)` ресабмит в ту же календарную минуту — разные суммы/тикеры не ограничены. Угнанный JWT может разместить много разных ордеров.
- [ ] **`App.jsx:1549` / `Markets.jsx:401` / `UpgradeModal.jsx:74` — `supabase.functions.invoke()` вместо `src/lib/callEdgeFunction.js` (найдено 2026-09-06, security-auditor, informational).** Не security и не CORS-баг (`invoke` добавляет `x-client-info`, который уже в `BASE_ALLOW_HEADERS`), но будущее изменение заголовков в `callEdgeFunction` эти три места не покроет. Нарушение конвенции CLAUDE.md.
- [ ] **CORS-поведение edge-функций не покрыто автотестами (найдено 2026-09-06, оба ревьюера, партии 2–3).** `npm run test:edge` запускается с `--no-check` (тайпчека нет), CI-воркфлоу — только codeql/dependency-review/gitleaks/semgrep, Deno-джобы нет. `ai-chat`/`stock-ai-analysis`/`get-insights` и др. `Deno.serve`-функции не используют тестируемый паттерн (`export async function handler` + `if (!Deno.env.get('ARK_EDGE_TEST')) Deno.serve(handler)`, как `alpaca-invest`/`plaid-sync-transactions`), поэтому `_test/` до них не дотянуться. Откат `const corsHeaders` обратно в module scope (то, что чинят эти партии) прошёл бы зелёным. Фикс: `_test/cors.test.ts` с проверкой резолвера (prod-эхо, preview-эхо, `evil.com`→fallback, `.evil.com`-суффикс→fallback, `Vary`+`Cache-Control` на всех) + перевод `Deno.serve`-функций на `handler`-паттерн, либо явно принять как accepted-risk.
...
- [ ] **Онбординг-тур, шаг 3 (`OnboardingFlow.jsx:15`, selector `[data-tutorial="ai-insight"]`) — подсвечивает полностью пустой элемент для нового юзера без данных.** Найдено 2026-08-03 при живом прогоне полного онбординга с нуля (одноразовый тестовый аккаунт, не demo). Шаг говорит "AI-powered insights — Tap to see personalized recommendations" (`tut_3_title`/`tut_3_desc`), но подсвечиваемый селектор — обёртка вокруг `<InsightCard insight={...} />` (`Dashboard.jsx:1427-1429`); для юзера без банка/транзакций `insight` пуст, `InsightCard` не рендерит вообще ничего видимого — тур подсвечивает пустую скруглённую полосу без единого слова текста. Тот же класс проблемы, что уже задокументированный шаг 1 ниже (текст тура не учитывает zero-data состояние), другой шаг того же тура. Не исправлено — как и шаг 1, вне скоупа этой сессии.
...
- [x] **Закрыто 2026-08-02: AI-чат подставлял месячный net (доход минус расход) вместо реального баланса счёта, когда баланс не закэширован — юзеру с $62,569 на счету AI сказал "-$1,085.61 — you're in overdraft right now".** Найдено в рамках QA-свипа demo-аккаунта после фикса multi-bank агрегации выше. `App.jsx`'s `sendChat()` строил AI-контекст через `currentBalance: plaidBalance ?? (totalIncome - totalSpent)` — `plaidBalance` читается из пассивного `localStorage`-кэша (`arkonomy_accounts_v2`, TTL 5 мин), который наполняется только побочным эффектом визита на Dashboard/Savings/онбординг-Aha-момент. Если AI-чат открыт первым (или кэш протух), `plaidBalance` был `null`, и в промпт молча уходил месячный net — совершенно другое число, ещё и с придуманным AI фреймингом "you're in overdraft". Воспроизведено живьём дважды (до и после фикса), детерминированно привязано к состоянию кэша, не случайность. **Фикс**: `sendChat()` теперь активно дёргает `plaid-get-accounts` при пустом кэше + подключённом банке — вместо пассивного доверия кросс-экранному кэшу; если баланс всё равно недоступен — в контекст уходит явный `null`, никогда не подставленное число. `ai-chat/index.ts`'s `DATA_BLOCK` (`BALANCE: $${metrics?.currentBalance ?? 0}`) заменён на явную инструкцию "unavailable, do NOT guess" при `null` — попутно найден и исправлен тот же класс бага: `?? 0` тоже был неявной подменой (показывал придуманный $0 вместо "неизвестно"). Добавлено правило `BALANCE HONESTY — NEVER VIOLATE` рядом с существующим `DEBT PAYOFF RULE`. **Верифицировано живьём, дважды**: (1) прямой вызов задеплоенного `ai-chat` с `currentBalance: null` → "Your account balance couldn't be loaded right now"; (2) после деплоя фронтенда — полный неорганизованный (не через тестовый скрипт) репро через реальный UI: очистка кэша → новый чат → реальный клик/тайп/сабмит → "$62,569", корректно.
- [x] **Закрыто 2026-08-02: `plaid-get-accounts` всегда бил в `production.plaid.com` с production credentials независимо от того, каким Plaid-окружением реально был подключён Item — demo-аккаунт (Sandbox) получал `accounts: []` на каждый запрос, из-за чего Dashboard/Cash Flow Forecast вечно висели в loading-skeleton.** Найдено тем же QA-свипом, до находки про AI-чат выше (это то, из-за чего изначально казалось, что фикс multi-bank агрегации "не показывает" $62,569 живьём). `plaid-sync-transactions` уже имел `plaid_environment`-guard, но он просто **пропускает** несовпадающие Items (не роняет batch), а не делает их реально рабочими — зеркалирование того же guard в `plaid-get-accounts` дало бы тот же результат (пустой массив, только без warning в логах). Реальный фикс — per-item `plaidBase`/`client_id`/`secret` вместо одного окружения на весь запрос: `PLAID_SANDBOX_CLIENT_ID`/`PLAID_SANDBOX_SECRET` уже существовали как Supabase secrets (добавлены 2026-07-30), но нигде не были подключены до этого фикса. Верифицировано живьём: `plaid-get-accounts` стал возвращать все 12 аккаунтов demo (6 depository) вместо 0; Dashboard/Insights полностью отрисовались вместо вечного skeleton.
- [ ] **`Savings.jsx:201` вызывает несуществующий i18n-ключ `savings.start_saving_mo` — рендерится буквально как `savings.start_saving_mo` на экране Savings (дважды: под каждой goal-карточкой).** Найдено QA-свипом demo-аккаунта 2026-08-02. Ключ реально существует только под `insights.start_saving_mo` (используется в `Insights.jsx:307`, идентичная семантика и `{{amount}}`/`{{date}}` интерполяция) — под `savings`-неймспейсом во всех 4 локалях его никогда не заводили. Не фикс-критично для релиза, но видимый юзеру (и Apple-ревьюеру) сырой ключ вместо текста. Путь фикса: либо переиспользовать `t("insights.start_saving_mo", ...)` в `Savings.jsx:201` (та же семантика, уже есть прецедент переиспользования ключей между секциями в этом проекте), либо завести отдельный `savings.start_saving_mo` во всех 4 locale-файлах.
- [ ] **Add Transaction — сабмит с `$0.00` и без выбранной категории молча ничего не делает, без единого сообщения об ошибке.** Найдено QA-свипом demo-аккаунта 2026-08-02 (проверка error-состояний, не только happy path). Клик "Add Expense" с пустыми обязательными полями не создаёт транзакцию (подтверждено — счётчик "All 34" не изменился), но и не показывает юзеру, какое поле нужно заполнить — модалка просто остаётся на месте. Не проверено, есть ли валидация ДРУГИХ некорректных состояний (отрицательная сумма, будущая дата и т.п.) — только пустой amount/category. Мелкий UX-гэп, не блокер релиза.
- [ ] **`handleDelete` (Transactions.jsx:497) — тот же класс бага, что найденный и исправленный в `handleDuplicate` (2026-08-14): `onDelete(id); toast(...)` вызывает toast синхронно, без ожидания результата и без проверки успеха.** Отличие от Duplicate-бага — здесь `onDelete` действительно вызывает правильную функцию (`deleteTransaction`), просто она (как и было до фикса `addTransaction`/`addSaving`/`editSaving`/`saveProfile`) ничего не возвращает — `deleteTransaction` (App.jsx) молча проглатывает `error` из ответа Supabase и не сообщает вызывающему коду об успехе/неудаче. Итог: если удаление реально упадёт на бэкенде, юзер всё равно увидит "Deleted" toast, а транзакция останется в списке. Тот же паттерн фикса, что уже применён трижды (#1 saveProfile, #2 addSaving/editSaving, #3 addTransaction) — вернуть true/false из `deleteTransaction`, дождаться результата в `handleDelete`, показывать toast только на подтверждённый успех. Осознанно не в скоупе фикса #3 (Duplicate) — отдельная задача, после текущих high-priority фиксов.
- [ ] **Markets → Explore Stocks поиск не имеет "No results found" состояния — при вводе несуществующего тикера просто ничего не происходит.** Найдено QA-свипом demo-аккаунта 2026-08-02. Подтверждено, что сам поиск функционален (валидный тикер "AAPL" корректно показывает результат "APPLE INC · Common Stock") — но для заведомо невалидного ввода ("ZZZQQQINVALID123") нет вообще никакой обратной связи: ни спиннера, ни честного "ничего не найдено", просто пропадает секция Trending/Sectors и ничего не появляется взамен. Юзер не может отличить "поиск завис" от "по этому запросу правда ничего нет". Мелкий empty-state гэп, не блокер релиза.
...
- [ ] **Ввести SPACING и shadow-токены проектно-широко** — сейчас отступы/тени везде ad-hoc пиксельные литералы, не специфично для Dashboard-редизайна, общая характеристика всего проекта. Найдено 2026-08-06 при проверке, унаследовал ли Dashboard-редизайн общие design-токены (`colors.js` экспортирует только `FONT`/`RADIUS`/`C`/`DASHBOARD_C`/`CAT_COLORS` — SPACING/shadow-константы никогда не существовали ни для одного экрана). Не срочно.
- [ ] **`GlassCard.jsx` (shared, 7 файлов) рендерит старый `C.card`/`C.border`, не DASHBOARD_C — включая уже смёрженный `Dashboard.jsx`.** Найдено 2026-08-09 при планировании переноса палитры на Savings.jsx — сам shared-компонент (`src/components/shared/GlassCard.jsx`) захардкожен на `C.card`/`C.border` и импортируется в 7 файлах (`App.jsx`, `Dashboard.jsx`, `Insights.jsx`, `Savings.jsx`, `Markets.jsx`, `Profile.jsx`, `AuthScreen.jsx`) — то есть уже смёрженный "готовый" Dashboard.jsx фактически рендерит карточки в старой холодной тёмно-синей палитре везде, где использует `<GlassCard>`, а не DASHBOARD_C. Не тронуто в рамках Savings.jsx — вместо изменения самого компонента, каждый мигрируемый экран передаёт `DC.card`/`${DC.faint}33` через уже существующий `style`-проп (спредится последним внутри компонента, override без риска для остальных 6 потребителей). Нужна отдельная миграция самого shared-компонента, после того как все 5 экранов индивидуально переведены на DC через override-стили.

**ai-chat промпт tech debt (аудит 11 июля, `buildSystemPrompt()` целиком) — не критично, отложено:**
- [ ] **Найдено 2026-08-26 (PR #49): LANGUAGE-секция флакует даже без tool_use.** Юзер репортнул: русское сообщение → ответ по-английски. Расследование нашло 3 реальных бага и все зафиксило (PR #49): (1) `profiles.preferred_language` физически не существовал в БД — select молча возвращал `undefined`, update падал без обработки; язык никогда не персистился на сервере, дефолт был жёстко `'en'` без auto-detect по браузеру; (2) приветствие чата (`buildContextGreeting`) было хардкодным английским текстом независимо от locale — ложный языковой сигнал перед первым реальным сообщением юзера; (3) override-правило ссылалось на "последнее сообщение", которое подменяется синтетическим `tool_result` после `web_search` — исправлено якорением на буквальный текст последнего юзер-сообщения, зафиксированный ДО входа в tool loop. **Но живое тестирование на том же сценарии (pro-аккаунт, однозначно русские вопросы про инвестиции) показало: `web_search` не сработал ни разу в 3 попытках (включая явный запрос live-цены акции — `stop_reason: end_turn` на первом шаге, подтверждено временным debug-логом, убран перед коммитом), и override всё равно сработал через раз (1 успех, 2 провала) БЕЗ какого-либо tool_use.** Это тот же класс проблемы, что уже решали понижением `temperature` до 0.4 (см. комментарий в `callWithToolLoop`) — тот фикс явно недостаточен сам по себе. Не решено намеренно — нужна A/B-оценка, не решение по одному ручному тесту. Два кандидата: (a) переформулировать LANGUAGE-секцию жёстче/проще — риск регресса других инструкций того же промпта; (b) вынести решение о языке в отдельный маленький детерминированный шаг перед основным вызовом — доп. латентность/cost на каждое сообщение. Дубль записи — см. `docs/known-issues.md`.
...
- [ ] **Найдено 2026-08-24: "Net Worth" — одна и та же подпись, две разные формулы, разный результат.** `Dashboard.jsx`'s `netWorth = accountBalance - totalDebt` (кэш минус долг по картам, без инвестиций/накоплений вообще) и `Savings.jsx`'s Asset Allocation total (cash + Stocks + Savings Goals, без вычета долга) оба подписаны "Net Worth", но считают структурно разные вещи. Найдено при работе над Asset Allocation fix (PR #39) — уже существовавшее расхождение, не внесено и не усугублено этим фиксом, не тронуто (вне скоупа задачи). Тот же класс проблемы, что уже несколько раз документирован в этом файле (один и тот же расчёт реализован независимо в разных местах и расходится) — нужна единая функция `calculateNetWorth()`, если оба места должны показывать сопоставимое число.
- [ ] **Pending Orders (Markets.jsx, PR #34) не пагинирует `/v2/orders?status=open`** — Alpaca отдаёт максимум 50 ордеров на запрос по умолчанию, `alpaca-portfolio` явно не передаёт `limit`/`page_token`. Осознанно не решено (комментарий в коде) — у этого приложения пока только единичные мелкие fractional-покупки, 50+ открытых ордеров нереалистично. Если использование вырастет — юзер с 50+ ордерами получит молча обрезанный список без единого признака, что что-то скрыто.
...
- **Savings.jsx собственный fetch аккаунтов на секунду показывает
  Cash $0 перед реальным значением** — найдено 2026-08-27 при живой
  проверке Step 2.5 (Net Worth unification). При первой отрисовке
  экрана Savings тайл "Cash" в Asset Allocation читает $0, затем
  через пару секунд обновляется на реальный баланс ($279 на живом
  проверенном аккаунте) — та же самая живая точка данных, что
  Dashboard получает без такой задержки в своём независимом fetch'е.
  Не исправлено: не баг данных (в итоге показывает верное число), тот
  же класс "известная нестабильность, не решена сейчас", что и
  LANGUAGE-флакинес в `docs/known-issues.md` — cosmetic race, не
  проверялась на реальных медленных соединениях. Вернуться, если
...
- **Unbounded `.select()` на `transactions` без явного `.limit()` — тот же
  класс бага, что был найден и исправлен в `generate-monthly-report` и
  `large-transaction-alert` (PostgREST default "Max Rows" = 1000, молча
  режет результат)** — найдено 2026-09-02 при аудите по запросу
  пользователя после фикса первых двух функций. Три места с датой-фильтром
  90 дней / 3 месяца, но без `.limit()`:
  `financial-diagnosis/index.ts:146` (`startOfLookback`, 90 дней),
  `get-insights/index.ts:183-190` (текущий месяц + предыдущие 3 месяца +
  income lookback), `push-notify/index.ts:230-233` (`LOOKBACK_DAYS = 90`).
  Не исправлено сейчас — на реальном аккаунте (~3 транзакции/день) 90-дневное
  окно даёт ~270 строк, далеко от порога в 1000, риск не актуален сегодня.
  Вернуться и применить тот же `.range()`-пагинационный паттерн, когда
  плотность транзакций пользователя вырастет настолько, что 90-дневное/
  3-месячное окно реально приблизится к 1000 строк (например, у
  активно использующих несколько банков/карт аккаунтов) — не ждать
  живой жалобы, как это случилось с `generate-monthly-report`.

---

## ✅ СДЕЛАНО 24 августа (для истории)

### Alpaca Portfolio UX — Holdings scale, Pending Orders, StockDetail owned position
- Разобрана жалоба "купил акцию, непонятно что происходит" — две независимые причины: (1) WST/SPCX корректно висели в `accepted`, просто ждали открытия рынка в понедельник (подтверждено через Alpaca `/v2/calendar`/`/v2/clock`, не баг — изначальный вывод "критический баг" отозван после перепроверки), но нигде в приложении не отображались — новая секция "Pending Orders" (`alpaca-portfolio` теперь дополнительно тянет `/v2/orders?status=open`). (2) Клик по Holdings уже открывал `StockDetail`, но терял `avg_entry_price`/`qty`/unrealized P&L — новая карточка "Your Position".
- Побочный фикс: шапка `StockDetail` использовала статичную карту ~18 тикеров для имени компании — любой другой тикер (включая все реально купленные: MP/WST/SPCX) показывал голый символ дважды. Теперь использует живой `stats.name`.
- Holdings + Watchlist + Pending Orders — все три ограничены top-3 + "+N more" (тот же паттерн, что у credit-cards на Dashboard).
- security-auditor нашёл и исправил 2 реальных бага до коммита: фиктивный "$0.00" при `notional:null`, захардкоженное "Buy" вместо реального `side`.
- **Процессный урок**: PR #35 был осознанно открыт стеком на ветке PR #34, но мерж PR с base = feature-ветка продвигает только эту ветку, не `main` — понадобился отдельный завершающий PR (#38), чтобы это дошло до main. GitHub не делает это автоматически.

### Asset Allocation — Stocks tile теперь учитывает Alpaca (PR #39)
- Было: только Plaid investment-аккаунты — Alpaca-холдинги (round-up + ручные покупки) никогда не учитывались, $0/0% независимо от реальных инвестиций.
- `investTotal` = баланс Plaid investment-счетов + сумма `market_value` по Alpaca-позициям (не `portfolio_value` — тот включает неинвестированный кэш на брокерском счёте). Добавлен guard от двойного счёта на случай, если один и тот же брокерский счёт когда-нибудь окажется привязан и через Plaid, и напрямую через OAuth.
- Живо подтверждено на реальном аккаунте: Stocks — с $0.00 до реальных значений.

### Round-up investing — выбор актива юзером (PR #45)
- Было захардкожено на SPY и в `investAlpaca()`, и в карточке подтверждения Savings.
- Новая колонка `profiles.roundup_symbol` (миграция, `DEFAULT 'SPY'`, CHECK-regex) — существующие юзеры не затронуты, подтверждено 17/17 строк с автозаполнением.
- Кнопка "Set as round-up investment" в Buy-табе StockDetail (переиспользует уже существующий флоу клика по любому тикеру, без нового пикера) — гейтится так же, как Buy (`!isPro || isTrial`).
- Карточка подтверждения в Savings показывает реальный выбранный тикер + ссылку "Change" обратно на Markets.
- Живая проверка end-to-end без реального ордера: выбор MP → подтверждено напрямую в БД → подтверждено отображение на карточке Savings.

### Dependabot — 5 смёржено, 5 проверено (ждут ручного мержа)
- Смёржено: #25-29 (`@capacitor/core`, `@typescript-eslint/parser`, `eslint`→10.8.1, `playwright`, `@playwright/test`).
- Проверено по отдельности (реальный `npm ci`/`build`/`eslint` на каждой ветке, не пакетно), все чистые, **ещё не смёржены** (нет прав на мерж через API): #40-44 (`@capacitor/android`, `eslint-import-resolver-node`→0.4.0, `eslint`→10.9.0, `firebase`→12.18.0, `supabase` CLI→2.115.0).
- #44 — это Supabase **CLI** (devDependency для `functions deploy`/локальной разработки), не `@supabase/supabase-js` (клиентский SDK, которым реально пользуется auth/RLS) — не перепутать в следующий раз. Breaking changes CLI 2.115 (движок `db diff`, `test db` fail-on-zero) не имеют blast radius — подтверждено, ни CI, ни scripts, ни git-хуки эти команды не вызывают.
- eslint 10 потребовал сначала смигрировать `eslint-plugin-import` → `eslint-plugin-import-x` (старый пакет никогда не получит релиз с поддержкой eslint 10) — живо подтверждено, что резолвер по-прежнему ловит реальные нарушения (посадил тестовое, поймал, откатил).

### Уборка worktree
- `.claude/worktrees/agent-a0219c24bf7ade1f7` — старая зарегистрированная git-worktree с прошлой сессии (та самая, где раньше уже убирали `storageState.json`, но сама папка осталась). Проверено перед удалением — реальных credentials не найдено (один скриншот с замаскированным паролем тестового `qa-audit-...@arkonomy.com` аккаунта, не утечка). Удалено через `git worktree remove --force`; заодно найдена и удалена осиротевшая локальная ветка с коммитом, уже смёрженным в `main` с 29 июля.
