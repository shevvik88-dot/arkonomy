...
- [x] **ЗАКРЫТО 2026-09-07. HIGH — `src/App.jsx` `deleteAccount()` не проверял `res.ok`, `catch` логировал только в DEV, дальше безусловно `signOut()` + сброс всего стейта** (найдено 2026-09-06 обоими ревьюерами при CORS-партии 2). `delete-account` легитимно возвращает 409 (инвестиция ещё pending), 500 (`admin.deleteUser` упал / не удалось проверить состояние) — во всех этих случаях юзер был разлогинен и видел «аккаунт удалён», хотя Stripe-подписка, Plaid-items и все строки на месте. Прямое нарушение правила CLAUDE.md про молчаливый `finally`, закрывающий UI независимо от результата; плюс делал любой CORS-сбой невидимым. **Фикс:** `deleteAccount()` теперь возвращает `{ ok, message }`: успех → `{ ok: true }` (сессия уже очищена); `res.ok === false` → ветвится: **401** → «сессия истекла, войдите снова» инлайн, БЕЗ `signOut` (он анмаунтит диалог до отрисовки сообщения — тот же silent-bounce, что чинили; юзер закрывает диалог и ре-аутентифицируется на следующем действии); **409** → показывает серверный `body.error` («an investment is still processing», self-healing race), без Sentry; **5xx/прочее** → `Sentry.captureException` + дженерик-сообщение. Сырой `e.message` (`TypeError: Failed to fetch`, текст FK-констрейнта) юзеру больше не показывается. `Profile.handleDeleteAccount` показывает `message` **инлайн внутри confirm-шита** (не app-level toast — тот рендерится `zIndex:300` под `zIndex:9000`-оверлеем, code-reviewer подтвердил headless-Chromium'ом), диалог закрывается только при `ok`, `deleteError` чистится при открытии/ретрае. `logger.error` — всегда (не только DEV; сам `logger.error` и так всегда пишет). **Live-проверено** одноразовым аккаунтом: seed `investments` row `status:'pending'` → `POST /delete-account` → HTTP 409 `{"error":"An investment is still processing…"}`, `getUser` после — 200, аккаунт и данные целы. Тот же класс, что открытый `handleDelete` (Transactions.jsx:497) выше — но это money/account-path, поэтому вынесен и закрыт первым.
- [ ] **Follow-up к фиксу `deleteAccount` (2026-09-07): 3 литерала ошибки захардкожены по-английски / 2 ключа** — `FAILED_MSG` + `SESSION_EXPIRED_MSG` в `App.jsx`, плюс fallback-строка в `Profile.jsx` (переиспользует `profile.delete_failed`). 4 locale-файла в тот момент правились параллельно под другую фичу, чтобы не мешать коммиты. Завести `profile.delete_failed` / `profile.delete_session_expired` во все 4 локали, заменить литералы на `t()`. Мелко.
- [ ] **Follow-up к фиксу `deleteAccount` (2026-09-07, code-reviewer): server-side `delete-account` не различает «ничего не удалено» и «данные удалены, auth.users остался».** `index.ts` удаляет `transactions/savings/…/profiles` (218–227), затем `admin.deleteUser` (229); при 500 на последнем шаге строки уже стёрты, а логин ещё работает → юзер залогинен в аккаунт без `profiles` (`usePlan` его читает). Клиент сейчас на любой не-2xx показывает «попробуйте ещё раз» (кроме 409 → серверный текст). Фикс на сервере: (a) флаг `retryable: true` на pre-delete 500 (`index.ts:126` «Could not verify account state» — ретрай реально помогает, а клиент шлёт в support); (b) на post-delete-500 клиент должен разлогинить; (c) `account_deletion_issues`-путь (Stripe/Plaid revoke упал) возвращает `{success:true}` без флага — добавить `partial: true`, чтобы клиент предупредил про возможно активную подписку. Проверено 2026-09-07: `merchant_aliases`/`scheduled_payments`.user_id → auth.users имеют `ON DELETE CASCADE` (`confdeltype='c'`), т.е. `admin.deleteUser` их подчищает каскадом — их отсутствие в явном `Promise.all` не баг; `investments.user_id` — `NO ACTION`, но он в явном списке.
