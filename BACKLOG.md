...
- [ ] **7 уже-мигрированных на allow-list функций без `Vary: Origin` (найдено 2026-09-06, оба ревьюера).** `check-bank-connection`, `plaid-get-accounts`, `market-data`, `auth-login`, `auth-signup`, `get-insights`, `generate-monthly-report` эхо-ят динамический origin, но не ставят `Vary: Origin` — промежуточный кэш теоретически может отдать CORS-заголовки одного origin на запрос другого (особенно `plaid-get-accounts`/`check-bank-connection` — возвращают балансы/имена банков). `financial-diagnosis`/`daily-lesson-v2`/`alpaca-portfolio` уже ставят. Закроется партией 5 (схлопывание инлайн-копий на `_shared/cors.ts`, который всегда ставит `Vary`).
