...
- [ ] **Не-constant-time сравнение service-role ключа в `plaid-sync-transactions` (найдено 2026-09-06, security-auditor).** `token !== Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')` на путях `resync_all` (глобально деструктивный — удаляет все Plaid-транзакции) и `sync_item`. Пре-существующее; remote timing side-channel на `!==` непрактичен, но это единственный гейт перед cross-tenant delete. `timingSafeEqual` по UTF-8 байтам — правка на две строки.
