...
export default function App() {
  ...
  // Financial Diagnosis Phase 2: lets a diagnosis action button land the
  // user directly in Savings' "New goal" modal, not just on the screen —
  // see Savings.jsx's autoOpenAdd/onAutoOpenAddConsumed.
  const [savingsAutoOpenAdd, setSavingsAutoOpenAdd] = useState(false);
  ...
  async function addTransaction(tx) {
    try {
      ...
      if (error) {
        ...
        return false; // keep the modal open (no finally-close) so the user's input isn't lost
      }
      if (data) {
  ...
  async function investAlpaca(data) {
    ...
    try {
      const { data: result, error } = await supabase.functions.invoke("alpaca-invest", {
        body: { amount: numAmount, symbol, operation_id },
      });
      ...
      // Never reached the server (or no response) — ambiguous. Leave the
      // key open; a retry reuses it.
  ...
  function openChatWithMessage(msg) {
    ...
    if (cached && Date.now() - cached.ts < CARD_ANSWER_CACHE_TTL_MS) {
      ...
      // trailing message is already this exact question, it's the same
