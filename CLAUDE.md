...
1. **Finish frontend Sentry test** — quick, independent: confirm `VITE_SENTRY_DSN` is in Vercel Production env vars, do an explicit Vercel deploy (git push alone doesn't deploy), then throw a real test error in the browser console and confirm it lands in the `arkonomy-web` Sentry dashboard.
...
4. **Audit remaining screens** — Transactions, Savings, Insights, Profile, Markets/StockDetail (BACKLOG #16)
5. **Weekly digest push** (BACKLOG #15)
6. **Apple Developer/Firebase for native push** — blocked, waiting on payment (salary)
7. **Demo account + screenshots** for App Store submission

Not touched recently, still open:
8. **Notification preferences UI** — Settings screen section: frequency selector + email digest content toggles; table `notification_preferences` already exists in Supabase
9. **E2E testing** — Playwright tests for critical flows (auth, bank connection, transaction add)
