...
- **Sentry error monitoring live** — frontend (`@sentry/react`, wraps existing `ErrorBoundary`) + backend (`ai-chat`/`get-insights` via `_shared/sentry.ts`, `initSentry`/`captureAndFlush`, `defaultIntegrations: false` + `Sentry.withScope()` per Supabase's official Deno guide). Scope-isolation between concurrent requests verified live (2 parallel test requests → 2 clean, non-leaking events). Root cause of the initial "events not arriving" symptom: `SENTRY_DSN` secret held a DSN for the wrong project ID (copy error at Sentry project creation), not a confirmed SDK/Deno-version incompatibility — that risk was flagged as a hypothesis going in and never actually confirmed. Full diagnostic trail in BACKLOG.md "СДЕЛАНО 13 июля". Still open: `VITE_SENTRY_DSN` needs adding to Vercel Production env vars manually; remaining edge functions beyond ai-chat/get-insights not yet covered.
...
- **Verify third-party DSNs/keys against the actual dashboard, not just "it was pasted"** — the Sentry `arkonomy-edge-functions` integration cost over an hour of diagnosis (secret checks, code review, network tests) chasing a suspected SDK/Deno-version incompatibility, when the real cause was a copy-paste error at project creation: `SENTRY_DSN` pointed at a different project ID than the one being watched in the dashboard. Sentry's ingest API returns 200 even for a wrong/nonexistent project ID (doesn't leak project existence), so `flush() === true` gave false confidence the whole time. Lesson: when a third-party integration silently "succeeds" but nothing shows up downstream, verify the credential/ID itself against the actual dashboard/console before assuming the SDK or network is at fault.
