...
- **Sentry error monitoring — backend live and verified, frontend written but untested.** Backend (`ai-chat`/`get-insights` via `_shared/sentry.ts`, `initSentry`/`captureAndFlush`, `defaultIntegrations: false` + `Sentry.withScope()` per Supabase's official Deno guide): scope-isolation between concurrent requests verified live (2 parallel test requests → 2 clean, non-leaking events in the dashboard). Root cause of the initial "events not arriving" symptom: `SENTRY_DSN` secret held a DSN for the wrong project ID (copy error at Sentry project creation), not a confirmed SDK/Deno-version incompatibility — that risk was flagged as a hypothesis going in and never actually confirmed. Frontend (`@sentry/react`, wraps existing `ErrorBoundary`): code is in place but never exercised in a real browser this session (no browser extension access) — still needs `VITE_SENTRY_DSN` confirmed in Vercel Production env vars, an explicit Vercel deploy (git push alone does not deploy — see project memory), and a real thrown-error test against the `arkonomy-web` dashboard. Remaining edge functions beyond ai-chat/get-insights intentionally not yet covered (incremental, not tech debt). Full diagnostic trail in BACKLOG.md "СДЕЛАНО 13 июля".
...
4. **Full RLS audit across all tables** (BACKLOG #13)
5. **Audit remaining screens** — Transactions, Savings, Insights, Profile, Markets/StockDetail (BACKLOG #16)
6. **Weekly digest push** (BACKLOG #15)
7. **Apple Developer/Firebase for native push** — blocked, waiting on payment (salary)
8. **Demo account + screenshots** for App Store submission

Not touched recently, still open:
9. **Port Deno-side recurring detectors to alias-aware logic** — client-side is now on the single source (recurringSummary.js); server-side is on TWO separate old detectors that need porting: `_shared/recurringDetector.ts` (used by `get-insights`) and a fully inline copy inside `push-notify/index.ts` itself (does not import the shared one). Deno can't import from `src/`, so each needs its own porting pass, not a quick fix.
