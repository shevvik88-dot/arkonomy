...
5. **Credit card debt as an explicit tier in PRIORITY ORDER** (ai-chat prompt tech debt) — currently only covered by the general `INTEREST CHARGES THIS MONTH > $0` case, not a separately-named tier.
6. **Rate limiting / cost guard on ai-chat** (BACKLOG #14) — note before starting: `enforceRateLimit` (`_shared/rateLimit.ts`) is already applied to both `ai-chat` (20/hr) and `get-insights` (30/hr); verify whether this already satisfies the task before assuming more work is needed.
7. **Full RLS audit across all tables** (BACKLOG #13)
