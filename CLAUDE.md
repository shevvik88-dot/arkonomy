...
2. **Port `push-notify` to the new alias-aware `_shared/recurringDetector.ts`** — mirrors what just shipped in `get-insights` (2026-07-17); remove `push-notify`'s own inline `detectUpcoming()` once it imports the shared detector instead. Higher-risk than the get-insights port since push-notify sends real notifications directly to users — test carefully before relying on it in prod cron.
3. **Fix the same plain-object prototype-collision pattern in client-side `src/utils/recurringSummary.js`'s `groupTransactionsByMerchant`** — the Deno copy in `_shared/recurringDetector.ts` was switched from a plain object to a `Map` on 2026-07-17 (see Current status above); the client-side original it was ported from still has the same risk (a transaction description normalizing to exactly `"constructor"` collides with `Object.prototype`).
4. **`Dashboard.jsx:614` — Cash Flow Forecast has no empty-state fallback** when no bank is connected (`accountBalance === null`) — renders a permanent, unlabeled loading skeleton that never resolves and never prompts to connect a bank. Found 2026-07-17 by qa-explorer while attempting to live-verify the recurring-detector alias fix on a QA account with no Plaid connection.
5. **Full RLS audit across all tables** (BACKLOG #13) — not yet started.
6. **Credit card debt as an explicit tier in PRIORITY ORDER** (ai-chat prompt tech debt) — currently only covered by the general `INTEREST CHARGES THIS MONTH > $0` case, not a separately-named tier.
7. **Rate limiting / cost guard on ai-chat** (BACKLOG #14) — note before starting: `enforceRateLimit` (`_shared/rateLimit.ts`) is already applied to both `ai-chat` (20/hr) and `get-insights` (30/hr); verify whether this already satisfies the task before assuming more work is needed.
8. **Audit remaining screens** — Transactions, Savings, Insights, Profile, Markets/StockDetail (BACKLOG #16)
9. **Weekly digest push** (BACKLOG #15)
10. **Apple Developer/Firebase for native push** — blocked, waiting on payment (salary)
11. **Demo account + screenshots** for App Store submission

Not touched recently, still open:
12. **Notification preferences UI** — Settings screen section: frequency selector + email digest content toggles; table `notification_preferences` already exists in Supabase
13. **E2E testing** — Playwright tests for critical flows (auth, bank connection, transaction add)
...
- **Updated 2026-07-17: only ONE Deno-side recurring-payment engine still diverges, not two.** `_shared/recurringDetector.ts` (used by `get-insights` for `upcomingBills7d`/`cash_risk`) was rewritten as an alias-aware TS port of `recurringSummary.js` — see Current status above, live-verified. `push-notify/index.ts` still has its own separate, fully inline `detectUpcoming()` (does NOT import `_shared/recurringDetector.ts` — its own copy, comment says "mirrors src/recurringDetector.js", the client file deleted 2026-07-12), unchanged: 90-day lookback, keyword allow-list, no `merchant_aliases` awareness, misses genuinely one-time bills (need ≥2 occurrences). Porting `push-notify` to the shared detector is Next tasks #2 — deliberately not done in the same pass as `get-insights` since push-notify sends real notifications to users, higher risk than a single `get-insights` signal.
- **`groupTransactionsByMerchant` in client-side `src/utils/recurringSummary.js` uses a plain object keyed by transaction description — same prototype-collision class as the Deno copy just fixed.** A description that normalizes to exactly `"constructor"` would collide with `Object.prototype` there too, corrupting that merchant's group. Not fixed yet — same fix (`Map` instead of `{}`) should port back once picked up (Next tasks #3).
