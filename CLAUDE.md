...
- **Sentry error monitoring — backend covers all 10 high-priority edge functions, frontend still written-but-untested.** Backend: `ai-chat`, `get-insights`, `delete-account`, `stripe-checkout`, `plaid-sync-transactions`, `plaid-link-token`, `alpaca-invest`, `alpaca-portfolio`, `stripe-webhook`, `plaid-exchange-token`, `plaid-batch-sync`, `alpaca-oauth-callback` — every one via `_shared/sentry.ts` (`initSentry`/`captureAndFlush`), every one verified with a real event landing in the `arkonomy-edge-functions` dashboard, not just "code looks right." Scope-isolation between concurrent requests verified live on `ai-chat` (2 parallel test requests → 2 clean, non-leaking events); root cause of the original "events not arriving" symptom was a wrong DSN project ID (copy error at Sentry project creation), not a confirmed SDK/Deno-version incompatibility. 9 medium-priority functions remain uncovered (`weekly-report`, `push-notify`, `plaid-webhook`, `plaid-get-accounts`, `market-data`, `generate-monthly-report`, `stock-ai-analysis`, `check-bank-connection`, `auth-login`) — intentional, incremental, not tech debt. Frontend (`@sentry/react`, wraps existing `ErrorBoundary`): code is in place but never exercised in a real browser (no browser extension access this session) — still needs `VITE_SENTRY_DSN` confirmed in Vercel Production env vars, an explicit Vercel deploy (git push alone does not deploy — see project memory), and a real thrown-error test against the `arkonomy-web` dashboard. Full diagnostic trail in BACKLOG.md "СДЕЛАНО 13/14 июля".
...
2. **Extend Sentry to the 9 medium-priority edge functions** (BACKLOG #18 continuation) — `weekly-report`, `push-notify`, `plaid-webhook`, `plaid-get-accounts`, `market-data`, `generate-monthly-report`, `stock-ai-analysis`, `check-bank-connection`, `auth-login`. Same pattern as the 10 high-priority functions (all done 14 июля) — check structure per function before assuming the standard try/catch pattern applies.
3. **Credit card debt as an explicit tier in PRIORITY ORDER** (ai-chat prompt tech debt) — currently only covered by the general `INTEREST CHARGES THIS MONTH > $0` case, not a separately-named tier.
4. **Rate limiting / cost guard on ai-chat** (BACKLOG #14) — note before starting: `enforceRateLimit` (`_shared/rateLimit.ts`) is already applied to both `ai-chat` (20/hr) and `get-insights` (30/hr); verify whether this already satisfies the task before assuming more work is needed.
5. **Full RLS audit across all tables** (BACKLOG #13)
...
10. **Port Deno-side recurring detectors to alias-aware logic** — client-side is now on the single source (recurringSummary.js); server-side is on TWO separate old detectors that need porting: `_shared/recurringDetector.ts` (used by `get-insights`) and a fully inline copy inside `push-notify/index.ts` itself (does not import the shared one). Deno can't import from `src/`, so each needs its own porting pass, not a quick fix.
11. **Notification preferences UI** — Settings screen section: frequency selector + email digest content toggles; table `notification_preferences` already exists in Supabase
12. **E2E testing** — Playwright tests for critical flows (auth, bank connection, transaction add)
